...
## Recent changes — matching the requested flow

The app now follows this flow end-to-end:

**Landing → Get Started → Splash → Log In / Sign Up → Dashboard (Trips, Expenses, Polls, Q&A, Profile)**

- `index.html`'s main **Get Started** button now opens `splash.html` first (it used to skip
  straight to Sign Up). The splash screen still auto-skips to `home.html` if you're already
  signed in.
- The Trip Dashboard's tabs are now **Members / Expenses / Polls / Q&A** as four separate tabs
  (previously "Q&A & Poll" was combined into one tab).
- Added `profile.html` + `js/profile.js` — a real **Profile** screen (view/edit your display
  name, see your email, log out), which didn't exist before.
- The hamburger menu on **My Trips** now opens an actual menu (My Trips / Profile / Log Out)
  instead of just showing a toast message.
- `service-worker.js` was missing `splash.html` from its offline cache list — fixed, and
  `profile.html` / `js/profile.js` were added. Cache bumped to `v3` so installed PWAs pick up
  the update.

## PWA support

CampusTrip is now installable as a Progressive Web App.

- `manifest.json` defines the app name, icon, theme, standalone display, and start URL.
- `service-worker.js` caches the local app shell so the interface can open when temporarily offline.
- `js/pwa.js` registers the service worker and shows a small offline status banner.
- `assets/icon-192.png` and `assets/icon-512.png` are generated from the existing CampusTrip logo.
- `vercel.json` supplies appropriate headers for the service worker and manifest.

### Install on a phone

Deploy the project over HTTPS (Vercel or Firebase Hosting), open the deployed site in a supported mobile browser, then use the browser's **Add to Home Screen / Install app** option.

The PWA caches the interface, but Firebase Authentication/Firestore still require connectivity for live data operations. The app therefore does not attempt to make the entire Firebase-backed experience fully offline.

### Vercel

This is a static app and does not require a build command. Deploy the `campustrip` folder as the project root. The included `vercel.json` configures the PWA headers.
