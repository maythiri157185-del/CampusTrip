...
document.getElementById("createTripForm").addEventListener("submit", async (e) => {
  ...
  if (!name || !destination || !dateLabel) {
    errorEl.textContent =
      "Please fill in the trip name, destination, and dates.";
  ...
  try {
    ...
    batch.set(
      tripRef.collection("members").doc(currentUser.uid),
      {
        name: organizerName,
    ...
    batch.set(
      db.collection("tripCodes").doc(code),
      {
        tripId: tripRef.id
      }
    );
    ...
    window.location.href =
      `trip.html?id=${tripRef.id}&created=1`;
