...
function renderTripHero() {
  ...
  document.getElementById("tripOrganizer").textContent = `${currentTrip.organizerName || "—"} (Organizer)`;
...
function loadMembers() {
  tripRef().collection("members").orderBy("joinedAt", "asc").onSnapshot(
    (snap) => {
      if (snap.empty) {
        document.getElementById("membersList").innerHTML = `<div class="empty-state">No members yet.</div>`;
        return;
      }
      const rows = [];
      snap.forEach((doc) => {
        ...
        const canRemove = isOrganizer && doc.id !== currentUser.uid;
        rows.push(`
          <div class="member-row">
            <div class="avatar">${escapeHtml(initials(m.name))}</div>
            <div class="name">${escapeHtml(m.name)}<span class="sub">${escapeHtml(m.email || "")}</span></div>
            ${isOrg ? '<span class="badge">Organizer</span>' : (canRemove ? `<button class="badge remove" data-uid="${doc.id}">Remove</button>` : "")}
          </div>`);
      });
      document.getElementById("membersList").innerHTML = rows.join("");
      ...
      document.querySelectorAll(".badge.remove").forEach((btn) => {
        btn.addEventListener("click", async () => {
          ...
          try {
            const batch = db.batch();
            batch.update(tripRef(), { memberIds: firebase.firestore.FieldValue.arrayRemove(uid) });
            batch.delete(tripRef().collection("members").doc(uid));
            await batch.commit();
            showToast("Member removed.");
          } catch (err) {
            showToast("Couldn't remove member: " + err.message, "error");
          }
        });
      });
    },
...
document.getElementById("expenseForm").addEventListener("submit", async (e) => {
  ...
  try {
    await tripRef().collection("expenses").add({
      ...
      paidByName: currentProfile?.name || currentUser.email,
    ...
    btn.addEventListener("click", async () => {
      ...
      try {
        await tripRef().collection("questions").doc(qid).update({
          ...
          answeredByName: currentProfile?.name || currentUser.email,
...
document.getElementById("questionForm").addEventListener("submit", async (e) => {
  ...
  try {
    await tripRef().collection("questions").add({
      ...
      askedByName: currentProfile?.name || currentUser.email,
      createdAt: firebase.firestore.FieldValue.serverTimestamp(),
      answerText: null
    });
    e.target.reset();
